...
Page({
  ...
  goDetail(e) {
    wx.navigateTo({
      url: '/pages/publishDetail/publishDetail?id=' + e.currentTarget.dataset.id,
    })
  },
  refreshList() {
    this.setData({
  ...
  nextPage() {
    const search = this.data.searchVO;
    if (search.pageNo * search.pageSize >= search.total) {
      return;
    }
    this.data.searchVO.pageNo += 1;
    this.getPublishList();
  },
  getPublishList() {
    const searchVO = this.data.searchVO;
    const db = wx.cloud.database()

    db.collection('funnys').count().then(res => {
      this.data.searchVO.total = res.total;
    })

    db.collection("funnys")
      .skip((searchVO.pageNo - 1) * searchVO.pageSize)
      .limit(searchVO.pageSize)
      .get({
        success: (res) => {
          console.log(res)
          this.setData({
            publishList: this.data.publishList.concat(res.data)
          })
        }
      });
  },
